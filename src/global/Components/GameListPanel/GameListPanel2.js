    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!this.inited) {
            if (nextProps.gameCategories && nextProps.gameCategories.length > 0) {
                ...
                // 嘿嘿嘿，---------------------------------------
                if($.isEmptyObject(browserHistory.getCurrentLocation().query)){
                    console.log($.isEmptyObject(browserHistory.getCurrentLocation().query))
        ...
        for (var i = 0; i < this.props.games.rows.length; i++) {
            ...
            ret.push(
                <li key={i} className="gameItem wow gamesPic"
                    ...
                    <a><img className="gameImg" src={imgUrl} onClick={this.onClickGame.bind(this, g)}/>
                        <span className="game-text game-text-btn BGcolor-second color-second">{g.Title}</span>
                    </a>
                    <a className="game-fuceng" onClick={this.onClickGame.bind(this, g)}>
        ...
        if (counter !== undefined) {
            for (var i = 0; i < this.props.gameCategories.length; i++) {
                ...
                if(counter[gc.ID]>0){ 
                    ...
                    console.log('this.state.selCategoryId',this.state.selCategoryId)
                    ret.push(
                        <li key={gc.ID}
