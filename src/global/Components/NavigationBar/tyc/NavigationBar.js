...
class NavigationBar extends Component {
    constructor(){
        ...
        this.state = {
            ...
            ShowSecondNav:true, //控制div是否显示的条件
        };
    }
    // 导航
    renderNav(){
    ...
    navIterator(mainNavData,key){
        return(
            <li 
                key={key} 
                className={"li-"+key} // 后台传入ClassName hot代热      
                >
                <Link 
    ...
    intoGames(game){
        this.toPage(1,game)
        setTimeout(()=>{
            browserHistory.push("/games?tab="+game.Tag.replace("电子"," "));            
        })
    }
    // 电子API ， 这个写法比较蠢，电子页重构以前暂时这么处理
    toPage(pageNo = 1,game) {
        let filter = {};
        filter.GamePlatform = game.Tag.replace("电子"," ");
        filter.GameType = 4;
    ...
    renderScondNav(){
        ...
        return this.state.FirstNavData.Data.map((data,index)=>{   
            if(!this.props.user.token && data.Title.includes('188')){
                return null
            }
            return (
                <li key={index} onClick={this.handleClickGame.bind(this, data)} className={'li_' + index}>
                    <div className='itemName'>{data.Title}</div>
                    <div className='itemImg' style={{ "backgroundImage": `url(${window.config.prdImgUrl + data.BackgroundImgUrl})` }}></div>
    ...
    render() {
        return (
            <nav role="navigation" className="MainNavigation">
                ...
                {
                    // 有数据，并且ShowSecondNav值为true时渲染二级列表
