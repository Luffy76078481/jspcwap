...
export function FishCanvas(){
    var HAMMERHEAD_RENDERER = {
        ...
        setParameters: function () {
            this.$window = $(window);
            this.$fishAnim = $('#jsi-sea-fishAnim');
            this.width = this.$fishAnim.width();
            this.height = this.$fishAnim.height();
            this.context = $('<canvas />').attr({ width: this.width, height: this.height }).appendTo(this.$fishAnim).get(0).getContext('2d');
        ...
        bindEvent: function () {
            this.$fishAnim.on('mousemove', this.changeView.bind(this, false));
            this.$fishAnim.on('mouseout', this.changeView.bind(this, true));
        },
        changeView: function (toAdjust, event) {
            this.destinationX = event.clientX - this.$fishAnim.offset().left + this.$window.scrollLeft();
